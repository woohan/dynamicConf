...
recordFileName = '1213_'+confName+'.csv' # file that records the results
logFileName = '1213_'+confName # log file
...
totalMsgNum = getDic("globalparas")["total.num.messages"] # total number of messages sent from producer
...
def run_consumer():
    ...
    consumer = Consumer({'bootstrap.servers':'172.29.0.3:9092,172.29.0.4:9092,172.29.0.2:9092', 
        'group.id': str(uuid.uuid1()),
        'auto.offset.reset': 'earliest',})
    consumer.subscribe([topicName])
    # store received message ids in a file for analysis
    f1 = open('/home/apps/deliveryTest/recordData4/receivedIDs', 'w')
    ...
    numrec = receivedNum
    logger2.info('The total time to receive '+str(numrec)+' messages from topic is '+str(totalTime)+' seconds')
    logger2.info('The average time for each message is '+str(totalTime*1000/numrec)+' ms')

def analysis():
    logger2.info("Analyzing...") # analyze the results of duplicate, lost messages
    topicName = sys.argv[1]

    v1s = np.loadtxt('/home/apps/deliveryTest/recordData4/receivedIDs', dtype='int')
    with open('/home/apps/deliveryTest/parameters.json') as fp:
        paras = json.load(fp)
    delay = paras['network.delay']
    pktloss = paras['packet.loss']
    rttloss = 1-(1-float(pktloss)/100)**2
    ...
    pollTmout = paras['poll.timeout.ms']
    ...
    res_lst = map(str,  [pktloss, delay, rttloss, bufferKb, bufferMs, msgSize, batchSize, msgTmout, acks, retryNum, retryMs, 
            sum_v1s, sum_actual, sum_lost,
            per_lost, sum_redundant, per_redundant, sum_duplicate,
            per_duplicate, proTime, pollTmout])
    res_str = ",".join(res_lst)
    logger2.info('Result is: \n'+res_str)
    with open('/home/apps/deliveryTest/recordData4/'+recordFileName, "a") as fw:
        fw.write(res_str+"\n")
